...
typedef struct {
    ...
    int write_allocate;            // 쓰기 할당 여부
    int write_back;                // 쓰기 백 여부
...
Cache* initialize_cache(int num_sets, int blocks_per_set, int bytes_per_block, int write_allocate, int write_back) {
    ...
    cache->num_sets = num_sets;                        // 세트의 수 설정
    cache->blocks_per_set = blocks_per_set;            // 세트당 블록의 수 설정
    cache->bytes_per_block = bytes_per_block;          // 블록당 바이트 수 설정
    cache->write_allocate = write_allocate;            // 쓰기 할당 여부 설정
    cache->write_back = write_back;                    // 쓰기 백 여부 설정
...
int access_cache(Cache* cache, char type, unsigned int address, int* load_hits, int* load_misses, int* store_hits, int* store_misses, unsigned long long* total_cycles) {
    ...
    if (set->blocks[lru_index].valid && set->blocks[lru_index].dirty && cache->write_back) { // 교체할 블록이 유효하고 더티 비트가 설정되어 있으며 write-back 모드일 때
        *total_cycles += 100 * (cache->bytes_per_block / 4); // 메모리로 쓰기 백 사이클 증가
    }
    ...
    if (type == 's' && !cache->write_allocate) { // 쓰기 할당이 아닌 경우
...
int main() {
    int num_sets = 256;           // 세트의 수
    int blocks_per_set = 4;       // 세트당 블록의 수
    int bytes_per_block = 16;     // 블록당 바이트 수
    int write_allocate = 1;       // 쓰기 할당 모드 (1: write-allocate)
    int write_back = 1;           // 쓰기 백 모드 (1: write-back)
    const char* tracefile = "gcc.trace"; // 메모리 접근 트레이스 파일
    ...
    char type;             // 접근 타입 (로드 또는 스토어)
    unsigned int address;  // 메모리 주소
    int result;            // 결과 (사용하지 않음)
    int load_hits = 0, load_misses = 0; // 로드 히트 및 미스 카운트
    int store_hits = 0, store_misses = 0; // 스토어 히트 및 미스 카운트
    unsigned long long total_cycles = 0; // 총 사이클 수
    int total_loads = 0, total_stores = 0; // 총 로드 및 스토어 수
